...
IplImage *getBackground(CvCapture *capture, int proportion, int height, int width)
{
    ...
    double ***blue = initBackground(height, width, 256);
    double ***green = initBackground(height, width, 256);
    double ***red = initBackground(height, width, 256);

    IplImage *image;//, *gray;
    CvScalar s;
    int posBlue, posGreen, posRed;
    ...
    IplImage *fond = cvCreateImage(cvSize(width, height), IPL_DEPTH_8U, 3);
    cvZero(fond);

    int totalBlue, totalGreen, totalRed;
    int mediumBlue, mediumGreen, mediumRed;
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            for (int k = 0; k < 256; k++)
            {
                totalBlue += blue[i][j][k];
                totalGreen += green[i][j][k];
                totalRed += red[i][j][k];
            }

            // tim median
            mediumBlue = totalBlue / 2;
            mediumGreen = totalGreen / 2;
            mediumRed = totalRed / 2;
            ...
            {
                totalBlue += blue[i][j][k];
                if (totalBlue >= mediumBlue)
                {
                    break;
                }
            }

            s.val[0] = k; // median of channel blue

            // channel green
            for (k = 0; k < 256; k++)
            {
                totalGreen += green[i][j][k];
                if (totalGreen >= mediumGreen)
                {
                    break;
                }
            }

            s.val[1] = k; // median of channel green

            // channel red
            for (k = 0; k < 256; k++)
            {
                totalRed += red[i][j][k];
                if (totalRed >= mediumRed)
                {
                    break;
                }
            }

            s.val[2] = k; // median of channel red

            cvSet2D(fond, i, j, s);
        }
    }

    // giai phong background
    deleteBackground(blue, height, width);
    deleteBackground(green, height, width);
    deleteBackground(red, height, width);
